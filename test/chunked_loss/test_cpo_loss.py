...
class LigerLMHeadCPO(torch.nn.Module):
    def __init__(
        ...
        super().__init__()
        self.lin = torch.nn.Linear(in_features=H, out_features=V, bias=bias, dtype=dtype)
        self.cpo_loss = LigerFusedLinearCPOLoss(ignore_index=ignore_index, beta=beta, alpha=alpha)
...
def test_correctness(B, T, H, V, scalar, dtype, atol, rtol, bias, ignore_index, beta, alpha):
